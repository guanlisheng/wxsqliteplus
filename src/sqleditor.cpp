...
("abort add after all alter analyze and as asc attach autoincrement before "
   ...
   "end escape except exclusive explain fail float for foreign from full glob "
   "group having if ignore immediate in index initially inner insert instead "
   "integer intersect into is isnull join key left like limit match natural not "
   "notnull null number numeric of offset on or order outer plan pragma primary "
   "query raise real references reindex rename replace restrict right rollback "
   "row select set table temp temporary text then to transaction trigger union "
...
("auto_vacuum cache_size case_sensitive_like count_changes database_list "
   ...
   "locking_mode max_page_count page_size parser_trace read_uncommitted "
   "schema_version short_column_names synchronous table_info temp_store "
...
/*---------------------------------------------------------------------------*/
void wxSQLEditorBase::OnUndoClick(wxCommandEvent& event){Undo();}
/*---------------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------------*/
void wxSQLEditorBase::OnRedoClick(wxCommandEvent& event){Redo();}
/*---------------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------------*/
void wxSQLEditorBase::OnCutClick(wxCommandEvent& event){Cut();}
/*---------------------------------------------------------------------------*/
void wxSQLEditorBase::OnPasteClick(wxCommandEvent& event){Paste();}
/*---------------------------------------------------------------------------*/
void wxSQLEditorBase::OnClearClick(wxCommandEvent& event){Clear();}
/*---------------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------------*/
void wxSQLEditorBase::OnCopyClick(wxCommandEvent& event){Copy();}
/*---------------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------------*/
void wxSQLEditorBase::OnSelectAllClick(wxCommandEvent& event){SelectAll();}
/*---------------------------------------------------------------------------*/
